...
(function (global, factory) {
   ...
   * }
    ...
    function before(el, target) {
        parentDislodgeHidden(el)
        dislodgeHidden(el);
        target.parentNode.insertBefore(el, target);
    }

    function parentDislodgeHidden(el) {
        if(el.className.indexOf('hidden')==-1) return
        el.className = el.className.replace(' hidden','')
    }

    /**
     *  dislodge child class hidden
     * @param el
     */
    function dislodgeHidden(el){
        var arr  = el.querySelectorAll('.hidden');
        var temp = arr.length-1;
        if(temp==-1) return;
        for(; temp >= 0; temp--){
            arr[temp].className =  arr[temp].className.replace(' hidden','')
        }
    }
    /**
